...
import Constants from 'expo-constants';
    ...
    //     </svg>\
    //     '
    //     ret = 'data:image/svg+xml;charset=UTF-8;base64,' + base64.encode(svg);
    //     return ret;
    // }
    // let svg = this.buildsvg("test", marker.note)
    // <Image source={{uri:svg}} /> 
    ...
    _request_get_all_floater_infos = async () => {
        try {
            ...
            if (responseJson["succes"] == true) {
                ...
                this.setState({markers:this.getMyMarkers()});
                this.state.markers.push(this.getSharedMarkers());
            }
        }
        catch (error) {
            this.setState({
